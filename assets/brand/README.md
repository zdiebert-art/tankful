...
All vector files derive from the authored Illustrator source. The mark
geometry (`tankFul-icon.svg`) and the wordmark (Europa Grotesk SH Bold,
preserved as outlined paths) are the locked-in pieces. No tagline
currently — one will be added later, at which point the lockups can be
...
| `lockup-vertical.svg` | Mark above wordmark. Uses `currentColor`. | Splash screens, decks, posters — any time the brand needs to read at full strength. |
| `lockup-vertical-white.svg` | Same lockup, white. | Same use cases on dark surfaces. |
| `lockup-horizontal.svg` | Mark on left, wordmark on right. Uses `currentColor`. | Wide headers, email signatures, social-banner headers. |
...
- Marks: 1024×1109
- Wordmark: 2400×480
- Vertical lockups: 1600×1573
- Horizontal lockups: 2400×730
...
- **Tagline — TBD.** Removed from current lockups; will be added back
  once the tagline copy is locked in.
